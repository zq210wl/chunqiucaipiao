...
var notWinBackNum = 20; // 在第几把未中奖就开始从下一把跟投
...
var curRealProfitFlag = {
  idx: 0,
  money: profitFlags[0].money
};

function finding() {
  for (var i = preEndIdx; i < dataLen; i++) {
    for (var j = 0; j < POS_KEY_ARR.length; j++) {
        var findSame = false;
        for (var m = i; m > i - notWinBackNum; m--) {
          if (allDatas.tencent[m] && hasSame(allDatas.tencent[m].lottery, POS_KEY_ARR[j])) {
            findSame = true;
          }
        }
        if (!findSame) {
          nextStartIdx = i + 1;
          var printStr = `\n\n======== 从第[ ${nextStartIdx + 1} ]把开始跟投, 位置[ ${POS_ENGLISH_TO_CHINESE[POS_KEY_ARR[j]]} ], 目标[ ${curRealProfitFlag.money} ] ========\n`;
...
function bet() {
  console.log(`开始投注`);
  preBets = JSON.parse(JSON.stringify(defaultData));
  for (var i = nextStartIdx; i < dataLen; i++) {
    ...
    if (allBets.profit >= curRealProfitFlag.money) {
      console.log('目标完成,盈利:', allBets.profit);
      preEndIdx = allBets.index;
      var nextProfitFlags = profitFlags[curRealProfitFlag.idx + 1];
      if (nextProfitFlags) {
        curRealProfitFlag = {
          idx: curRealProfitFlag.idx + 1,
          money: Number((allBets.profit + nextProfitFlags.money).toFixed(2))
        };
        finding();
      } else {
