...
DIRS.forEach(function(dir){
  var dataArr = [];
  var dirsFiles = fs.readdirSync(`./${dir}/originData`);
  ...
  dirsFiles.forEach(function(fileName){
    ...
    dataArr = dataArr.concat(JSON.parse(fs.readFileSync(`./${dir}/originData/${fileName}`, 'utf8')).data.original_data);
  });
...
// 如果文件已经存在就先删除
if (fs.existsSync(`./tencent/originResult/${exportFilename}.txt`)) {
  fs.unlinkSync(`./tencent/originResult/${exportFilename}.txt`);
}
...
function printData() {
  ...
  fs.appendFile(`./tencent/originResult/${exportFilename}.txt`, printStr, function () {
